...
def buffer_get_offsets(total_size):
    ...
    for i in range(num_segments):
        ...
        offsets.append(f"{high_byte:02X}{low_byte:02X}")
...
def buffer_read_all(connection, offsets):
    ...
    byte_array = [(int(offset[:2], 16), int(offset[2:], 16)) for offset in offsets]

    for i in range(offset_count):
        high_byte, low_byte = byte_array[i]
        read_buffer_command = [0xEE, 0xF0, low_byte, high_byte]
...
def print_tokenized_lines(tokenized_output):
    for line in tokenized_output:
        if len(line) > 11 and all(byte == 0 for byte in line[:11]):
            continue
